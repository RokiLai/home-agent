...
| 维度 | 当前状态 | 说明 |
|---|---|---|
| 文档定位 | 历史 MVP 方案 | 保留早期中心化 SSH Push 方案及其决策背景，不再作为当前实施清单 |
| 已演进能力 | 已落地 | Registry、ACL、Agent 注册、公钥上报、Managed Block、原子写入和幂等同步已在现行架构中实现 |
| 已被取代部分 | 不再实施 | Server 持有 Admin SSH Key 主动登录所有设备的方向，已被 Agent 主动建立 SSE 控制通道和本地安全应用取代 |
| 剩余缺口 | 另案跟踪 | SSH 安全加固、旧 `ssh-access` 迁移、离线 KeySet 和灾难恢复见《[SSH 访问集成待办事项](../features/SSH访问集成待办事项.md)》 |

> 第 59 节清单是原始 MVP 验收基线，不代表现行架构的完成状态；其中 SSH Push 专属项目已废止，不应再作为待实现需求。
