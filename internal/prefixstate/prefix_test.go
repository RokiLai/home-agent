...
func TestPrefixNormalizationAndIntersection(t *testing.T) {
	...
	activePrefixes := []netip.Prefix{
		netip.MustParsePrefix("2001:db8:1234:10::/64"),
		netip.MustParsePrefix("2001:db8:1234:20::/64"),
	}

	candidates := []networkaddr.ReportedIPv6Address{
		{Address: "2001:db8:1234:10::100"},
		{Address: "2001:db8:1234:20::200"},
		{Address: "2001:db8:9999:99::999"}, // outside prefix
	}
...
func TestPrefixRevisionConflictCarriesAtomicSnapshot(t *testing.T) {
	svc := NewService(nil)
	now := time.Now().UTC()
	if _, _, err := svc.UpdateRouterPrefixes("router-1", "home", 9, now, nil); err != nil {
		t.Fatal(err)
	}
	_, _, err := svc.UpdateRouterPrefixes("router-1", "home", 1, now, nil)
...
func TestDualPrefixTransition_A_AB_B(t *testing.T) {
	...
	addrA := netip.MustParseAddr("2001:db8:10::1")
	addrB := netip.MustParseAddr("2001:db8:20::2")

	prefA := []netip.Prefix{netip.MustParsePrefix("2001:db8:10::/64")}
	prefAB := []netip.Prefix{netip.MustParsePrefix("2001:db8:10::/64"), netip.MustParsePrefix("2001:db8:20::/64")}
	prefB := []netip.Prefix{netip.MustParsePrefix("2001:db8:20::/64")}
...
func TestParseUbusInterfaceStatus(t *testing.T) {
	...
	if prefixes[0].Prefix != "2001:db8:1234:10::/64" {
		t.Errorf("expected 2001:db8:1234:10::/64, got %s", prefixes[0].Prefix)
	}
...
func TestPrefixService_RevisionsAndStale(t *testing.T) {
	...
	// 4. Stale check
	prefs, isStale, err := svc.GetActivePrefixes("home", st.LastSeenAt.Add(30*time.Minute), 15*time.Minute)
	if err != nil {
