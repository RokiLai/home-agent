...
func TestCloudflareClient_GetUpsertDelete(t *testing.T) {
	...
	handler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodPost && strings.Contains(r.URL.Path, "/zones/test-zone/dns_records"):
			records["rec-new"] = "240e:10::2"
			w.Write([]byte(`{"success":true,"result":{"id":"rec-new","type":"AAAA","name":"node.example.com","content":"240e:10::2","ttl":120},"errors":[]}`))

		case r.Method == http.MethodPut && strings.Contains(r.URL.Path, "/zones/test-zone/dns_records/rec-1"):
			records["rec-1"] = "240e:10::2"
			w.Write([]byte(`{"success":true,"result":{"id":"rec-1","type":"AAAA","name":"node.example.com","content":"240e:10::2","ttl":120},"errors":[]}`))
	...
	if len(addrs) != 1 || addrs[0].String() != "240e:10::1" {
		t.Fatalf("expected 240e:10::1, got %v", addrs)
	}

	// 2. UpsertAAAA (update existing rec-1)
	newIP := netip.MustParseAddr("240e:10::2")
	if err := client.UpsertAAAA(ctx, "node.example.com", newIP, 120); err != nil {
		t.Fatalf("UpsertAAAA failed: %v", err)
	}
	if records["rec-1"] != "240e:10::2" {
