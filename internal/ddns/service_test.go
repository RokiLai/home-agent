...
func TestDDNSService_LifecycleAndTransitions(t *testing.T) {
	...
	// 1. Router reports prefix A (2001:db8:10::/64)
	...
	// 2. MacBook reports candidate A (2001:db8:10::1)
	...
	// 3. Reconcile macbook-1 -> should publish AAAA 2001:db8:10::1
	if err := ddnsSvc.ReconcileDevice(ctx, "macbook-1"); err != nil {
		t.Fatalf("reconcile failed: %v", err)
	}

	st, _ := devSvc.Get("macbook-1")
	if st.SyncStatus != devicestate.SyncStatusSynced || st.AppliedAddress != "2001:db8:10::1" {
		t.Fatalf("expected synced with 2001:db8:10::1, got status=%s, applied=%s", st.SyncStatus, st.AppliedAddress)
	}
	if pub.records["macbook.example.com"].String() != "2001:db8:10::1" {
	...
	// Router drops A and now only has B (2001:db8:20::/64)
	...
	// Device reports A and B (2001:db8:10::1 and 2001:db8:20::2)
	candAB := []networkaddr.ReportedIPv6Address{
		{Address: "2001:db8:10::1", Interface: "en0"},
		{Address: "2001:db8:20::2", Interface: "en0"},
	}
	...
	st, _ = devSvc.Get("macbook-1")
	if st.SyncStatus != devicestate.SyncStatusSynced || st.AppliedAddress != "2001:db8:20::2" {
		t.Fatalf("expected synced with 2001:db8:20::2, got status=%s, applied=%s", st.SyncStatus, st.AppliedAddress)
	}
	if pub.records["macbook.example.com"].String() != "2001:db8:20::2" {
		t.Fatalf("expected 2001:db8:20::2 in publisher, got %v", pub.records)
	}
