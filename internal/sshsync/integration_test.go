...
func TestThreeDeviceAllowAllManagedBlocks(t *testing.T) {
	...
	for _, target := range all {
		...
		if !strings.Contains(text, target.PublicKey+" "+target.ID) {
			t.Fatalf("target %s missing its own key", target.ID)
		}
		for _, allowed := range policy.Resolve(target.ID, all) {
		...
		hash := ComputeKeySetHash(keys)
		if expectedHash == "" {
			expectedHash = hash
		} else if hash != expectedHash {
			t.Fatalf("target %s hash %s differs from %s", target.ID, hash, expectedHash)
		}
	}
}
...
func TestSSHPathOptionQuotesSpacesAndSpecialCharacters(t *testing.T) {
	got, err := sshPathOption("UserKnownHostsFile", `/Users/example/Library/Application Support/HomeAgent/ssh/known"hosts`)
	if err != nil {
		t.Fatal(err)
	}
	want := `UserKnownHostsFile="/Users/example/Library/Application Support/HomeAgent/ssh/known\"hosts"`
	if got != want {
