...
// EnsureAdminKey 检查是否存在现有的 ed25519 管理员密钥对，若不存在则自动生成全新密钥对。
// 返回管理员公钥字符串。
...
// Controller 协调通过出站 SSH 连接主动向目标设备分发公钥。
...
// Result 记录单台设备执行 SSH 公钥同步的结果与状态。
type Result struct {
...
// SyncAll 并发触发对所有已注册设备的 SSH 公钥同步。
...
// SyncDevice 通过 SSH 管道将 ACL 允许的公钥同步写入单台目标设备。
