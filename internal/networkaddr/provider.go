...
// AddressProvider 定义探测本机网络接口上 IPv6 地址的接口规范。
...
// GenericProvider 是使用 Go 标准库 net.Interfaces() 的跨平台默认地址探测提供器。
type GenericProvider struct{}

// NewGenericProvider 创建通用网络接口地址提供器。
func NewGenericProvider() *GenericProvider {
	return &GenericProvider{}
}

// GetAddresses 扫描本地活动网络接口并提取所有非回环 IPv6 地址。
...
// ParseDarwinIfconfig 解析 macOS `ifconfig <interface>` 命令的输出以提取完整的 IPv6 地址与标志属性。
func ParseDarwinIfconfig(output string, ifaceName string) []ReportedIPv6Address {
	...
	for _, line := range lines {
		...
		// Format example: inet6 2001:db8:1234:10::20 prefixlen 64 [scopeid 0x4] [flags]
