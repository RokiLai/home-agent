...
func TestNormalizeAndFilterCandidates(t *testing.T) {
	...
	input := []ReportedIPv6Address{
		...
		{Address: "2001:db8:9999::2", Interface: "en0"},                        // valid 2nd
		{Address: "2001:db8:1111::1", Interface: "en0"},                        // valid 1st
		{Address: "2001:0db8:1111:0000:0000:0000:0000:0001", Interface: "en0"}, // duplicate of 1st, normalize & dedup
		{Address: "2001:db8:1111::1", Interface: "en0", Temporary: true},       // temp, reject
	}
	...
	if result[0].Address != "2001:db8:1111::1" {
		t.Errorf("expected first to be 2001:db8:1111::1, got %s", result[0].Address)
	}
	if result[1].Address != "2001:db8:9999::2" {
		t.Errorf("expected second to be 2001:db8:9999::2, got %s", result[1].Address)
	}
}
...
func TestAddressesEqual(t *testing.T) {
	...
	a := []ReportedIPv6Address{
		{Address: "2001:db8:1::1", Interface: "en0", PreferredUntil: &t1},
	}
	b := []ReportedIPv6Address{
		{Address: "2001:db8:1::1", Interface: "en0", PreferredUntil: &t1},
	}
	c := []ReportedIPv6Address{
		{Address: "2001:db8:1::1", Interface: "en0", PreferredUntil: &t2},
	}
