...
func TestDefaultVersionHasSingleSource(t *testing.T) {
	if defaultServerVersion != "v0.6.20" || defaultAgentVersion != "v0.6.15" {
		t.Fatalf("defaults = %q/%q, want v0.6.20/v0.6.15", defaultServerVersion, defaultAgentVersion)
	}
	...
	if !strings.Contains(string(source), `const defaultServerVersion = "v0.6.20"`) {
